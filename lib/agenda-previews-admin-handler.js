...
async function addBlankPage(admin, actorId, payload) {
  ...
  if (existing) {
    if (existing.preview_id !== preview.id || existing.page_type !== 'blank') {
      throw new ApiError(409, 'page_id_conflict', 'El identificador de página ya está en uso.');
    }
    return { page: existing, preview };
  }
  const inserted = await insertAtPosition(
    admin,
    actorId,
    preview.id,
    { id: pageId, page_type: 'blank' },
    payload?.position
  );
  return {
    page: inserted.page,
